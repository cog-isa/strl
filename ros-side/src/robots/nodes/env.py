...
def dist(p1, p2):
	...
	len=math.sqrt( (p1.position.x - p2.position.x)**2 + (p1.position.y - p2.position.y)**2)
	phi = getAngleBetweenPoints(p1.position.x,p1.position.y,p2.position.x,p2.position.y)
	return [len,phi]
